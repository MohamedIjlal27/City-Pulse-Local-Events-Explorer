      ...
      <div className="flex min-h-screen items-center justify-center bg-zinc-50 dark:bg-black px-4 py-8">
        <div className="w-full max-w-md">
          <div className="bg-white dark:bg-zinc-900 rounded-lg shadow-lg p-6 sm:p-8 text-center">
            ...
            <p className="text-sm sm:text-base text-zinc-600 dark:text-zinc-400 mb-6 break-words">
              {t('emailSentTo')} <strong className="break-all">{email}</strong>
            </p>
            <p className="text-xs sm:text-sm text-zinc-500 dark:text-zinc-400 mb-6 sm:mb-8">
              {t('clickLinkInEmail')}
            </p>
            <button
              onClick={() => {
              ...
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 sm:py-3 px-4 rounded-lg transition-colors mb-4 text-sm sm:text-base"
            >
              {t('sendAnotherEmail')}
            </button>
            <Link
              href="/login"
              className="block text-center text-xs sm:text-sm text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300"
            >
              {t('backToLogin')}
            </Link>
          </div>
